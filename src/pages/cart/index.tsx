...
const Cart = () => {
  ...
  } = api.group.getGroupByGroupCode.useQuery({
  ...
  const total_items = productsInCart.reduce(
    (accumulator, product) => accumulator + product.quantity!,
    0
  );
  const totalPrice = productsInCart.reduce(
    (accumulator, product) =>
      accumulator + product.retail_price * product.quantity!,
    0
  );
            ...
            {joinedGroup ? (
              <div>
                ...
                <div className="mt-3 flex items-center gap-x-2">
                  <Image
                    src={`https://api.gr-oops.com/${joinedGroup?.group_image_url}`}
                    width={100}
                    height={100}
                    alt={`${joinedGroup?.group_image_url}`}
                  />
                  <div className="flex flex-col">
            ...
            <div>
              {productsInCart.length > 0 ? (
                ...
                <div>
                  ...
                  <p>
                    Price: <CurrencyFormatter amount={totalPrice} />
                  </p>

                  <div className="mb-5 flex justify-center p-5">
                    {joinedGroupID?.group_code ? (
                      <button
                        className="w-[90%] rounded-lg bg-rose-600 p-3 text-white"
                        onClick={handlePlaceOrder}
                      >
                        Place Order
                      </button>
                    ) : (
                      <Link href="/group" className="text-sm text-red-500">
                        Please join a group first to check out
                      </Link>
                    )}
                  </div>
                </div>
