...
const Orders = () => {
  ...
  const { data: orders, isLoading: loadingOrders, refetch } =
      ...
      <div className="flex justify-between">
        ...
        <div className="flex gap-x-10">
          <div className="flex flex-col">
            ...
            <div className=""> {order.order_date ? (
  ...
  // layout="responsive"
  objectFit="cover"
  className="w-36 h-28" // Apply additional Tailwind CSS classes for styling if needed
/>
</div>
        ))
        }
        ...
        <div className="flex w-1/2 items-center justify-end gap-x-10">
          <div className="flex flex-col ">
            ...
            <span className=""><CurrencyFormatter amount={order.sub_total??0}/></span>
          </div>
          ...
          onClick={()=>{handleCancelOrder(order.id)}}>cancel</button>
          </div>
        </div>
      </div>
    </div>
